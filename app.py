...
div[data-testid="stSidebar"] button[kind="primary"] {
    background:linear-gradient(90deg,#103b67,#0d2942) !important;
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-section">OPTIMIZATION</div>', unsafe_allow_html=True)
    nav_items += [
        ("📦", "RM Stock"),
        ("🥧", "Burden Composition"),
        ("₹", "Cost Composition"),
...
def dashboard_page():
    # Compact controls
    top1, top2, top3 = st.columns([4.4, 1.55, 1.55])
    ...
    if st.session_state.get("show_uploader", False):
        st.markdown(
            '<div class="edit-hint">Optional Excel — standby master chemistry. '
            'Built-in chemistry remains active until you choose the uploaded file.</div>',
            unsafe_allow_html=True
        )
        up = st.file_uploader(
    ...
    if st.session_state.inputs_changed:
        st.markdown(
            '<div class="banner-warn">✏ Inputs changed — click RUN OPTIMIZER to '
            'refresh burden, chemistry and cost.</div>',
            unsafe_allow_html=True
        )
    ...
    cards = [
        ...
        ("kpi-amber", "ACHIEVED Fe", f"{achieved['Fe']:.2f} %",
         f"Target {FE_LOWER:.1f}–{FE_UPPER:.1f}%"),
        ("kpi-cyan", "SOLUTION STATUS",
         "Optimal" if result["status"] == "Optimal" else "Review",
         "Optimization successful" if result["status"] == "Optimal" else "Quality relaxed"),
    ...
    # Keep only ONE Achieved / Target table on the dashboard.
    quality_banner(achieved)

    # Main row: commercial inputs | burden donut | final chemistry
    a, b, c = st.columns([1.20, 0.95, 1.20])
    ...
    with a:
        ...
        st.markdown(
            ...
            '&nbsp;&nbsp; 🔒 Chemistry / Tech Max read-only</div>',
            unsafe_allow_html=True
        )
        ...
        for m in df.index:
            rows.append({
                ...
                "Status": "Available"
                if st.session_state.availability.get(m, True)
                and df.loc[m, "Available_Tonnes"] > 0 else "Unavailable",
            })
        ...
        edited = st.data_editor(
            ...
            column_config={
                "Available": st.column_config.CheckboxColumn(
                    "Available",
                    help="Toggle whether this material is available for the optimizer."
                ),
        ...
        for _, row in edited.iterrows():
            ...
            if float(row["Price (₹/t)"]) != float(df.loc[m, "Price_Rs_t"]):
                df.loc[m, "Price_Rs_t"] = float(row["Price (₹/t)"])
                changed = True
            if float(row["RM Stock (t)"]) != float(df.loc[m, "Available_Tonnes"]):
                df.loc[m, "Available_Tonnes"] = float(row["RM Stock (t)"])
                changed = True
            if bool(row["Available"]) != bool(st.session_state.availability.get(m, True)):
                st.session_state.availability[m] = bool(row["Available"])
                changed = True
        if changed:
    ...
    with b:
        ...
        fig = px.pie(
            chart_df, names="Label", values="kg/t", hole=.58, color="Label",
        ...
        fig.update_layout(
            ...
            annotations=[dict(
                text=f"<b>{total_burden:,.1f}</b><br>kg/t",
    ...
    with c:
        ...
        st.dataframe(
            q[["KPI", "Achieved", "Target", "Status"]],
            hide_index=True,
            use_container_width=True,
            height=315
        )

    # Freed space: wider chemistry + optimal burden/cost tables.
    ...
    with left:
        ...
        chem = r_df.reset_index()[
            ["Material", "Group", "Fe", "SiO2", "Al2O3", "CaO", "MgO", "LOI"]
        ].copy()
        for col in ["Fe", "SiO2", "Al2O3", "CaO", "MgO", "LOI"]:
            chem[col] = chem[col].round(3)
        st.dataframe(chem, hide_index=True, use_container_width=True, height=330)
    ...
    cost_cards = [
        ...
        ("IRON ORE COST", group_costs["Iron_ore"],
         group_costs["Iron_ore"] / total_cost * 100 if total_cost else 0, "kpi-blue"),
        ("FLUX COST", group_costs["Flux"],
         group_costs["Flux"] / total_cost * 100 if total_cost else 0, "kpi-green"),
        ("RECYCLE COST", group_costs["Recycle"],
         group_costs["Recycle"] / total_cost * 100 if total_cost else 0, "kpi-amber"),
        ("FUEL COST", group_costs["Fuel"],
         group_costs["Fuel"] / total_cost * 100 if total_cost else 0, "kpi-purple"),
    ]
    cols = st.columns(5)
